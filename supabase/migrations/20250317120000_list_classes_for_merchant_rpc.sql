-- 分站課程列表：分頁 + 搜尋 + 主題分類 + 日期（class_date 或 scheduled_slots）+ 年齡區間（與 sidebar_option 對齊）
-- 由 Server Action 以 service role 呼叫，p_merchant_id 必須來自環境變數隔離。
...
comment on function public.list_classes_for_merchant_page is
  '分站課程列表分頁：merchant 隔離、標題搜尋、marketplace_category、日期（class_date 或 scheduled_slots）、年齡與 sidebar_option 0/1/2/3 對齊。';
