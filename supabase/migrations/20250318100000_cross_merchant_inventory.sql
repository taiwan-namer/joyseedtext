-- 總站列表課程可將「庫存／場次／訂單」指向老師端課程（同一 Supabase、不同 NEXT_PUBLIC_CLIENT_ID）。
-- listings：classes.inventory_merchant_id + inventory_class_id
-- bookings：merchant_id/class_id 為庫存擁有者；sold_via_merchant_id 為結帳網站商家（可選）

alter table public.classes
  add column if not exists inventory_merchant_id text,
  add column if not exists inventory_class_id uuid;

comment on column public.classes.inventory_merchant_id is '庫存所屬商家 id（老師）；與 inventory_class_id 同時有值時，報名扣名額與訂單歸屬該課程';
comment on column public.classes.inventory_class_id is '庫存所屬課程 id（老師 classes.id）';

alter table public.bookings
  add column if not exists sold_via_merchant_id text;

comment on column public.bookings.sold_via_merchant_id is '經由哪個商家網站結帳（例如總站）；merchant_id 仍為庫存擁有者';

-- ---------------------------------------------------------------------------
-- create_booking_and_decrement_capacity：解析庫存課程後再檢查名額／寫入訂單
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_slot_date is not null and p_slot_time is not null then
    ...
    insert into bookings (
      ...
      payment_method, order_amount, sold_via_merchant_id
    )
    values (
      v_inv_merchant,
      p_member_email,
...
comment on function public.create_booking_and_decrement_capacity is
  '下單：支援列表課程 inventory_* 指向他店庫存；訂單 merchant_id/class_id 為庫存課程，sold_via 為結帳站。名額僅計入 paid/completed。';

-- ---------------------------------------------------------------------------
-- create_booking_from_pending：同上，由 pending 的列表課程解析庫存
-- ---------------------------------------------------------------------------
...
comment on function public.create_booking_from_pending is
  '從 pending 建立 paid 訂單：支援 inventory_*；訂單歸庫存擁有者，sold_via 為列表站。無場次時 atomic 扣庫存課程名額。';
