-- 名額改為「付款成功才佔用」：建立訂單時不扣名額，僅在 status 為 paid/completed 時計入名額。
-- 這樣選 LINE Pay / 綠界 / 藍新 後未完成付款不會佔掉名額。
...
begin
  ...
  -- 無場次：僅在「非線上金流」時立即扣 classes.capacity；linepay/ecpay/newebpay 等付款成功後再扣（由 callback 處理）
...
comment on function public.create_booking_and_decrement_capacity is '下單：名額僅計入已付款；有 slot 時依場次 paid 數判斷，無 slot 且非線上金流時才扣 classes.capacity，線上金流於 callback 付款成功時再扣';
