...
export default function AdminRescheduleModal({ bookingId, open, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    getReschedulePreviewAsAdmin(bookingId).then((res) => {
      if (cancelled) return;
      setLoading(false);
      if (!res.success) {
        setError(res.error);
  ...
  const handleConfirmSlot = async (date: string, time: string) => {
    ...
    setError(null);
    try {
      const res = await rescheduleBookingAsAdmin({
        bookingId,
        newSlotDate: date,
        newSlotTime: time,
      });
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
            ...
            <button
              type="button"
              onClick={onClose}
              className="mt-6 w-full rounded-lg bg-amber-600 py-2.5 text-sm font-medium text-white hover:bg-amber-700"
            >
              關閉
            </button>
          </div>
